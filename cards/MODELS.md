...
- **Timeline** is a game's shared timeline (one per game, `OneToOne` to Game, created on demand). Holds a `title` and `notes` shown above the cards. Not an AliveMixin model — edited through the timeline page, not a CRUD list.
- **TimelineCard** is a note card (title + notes) on a Timeline. All game members can create, place, edit, and delete them. Top-level cards (`depth` 0) sit in a horizontal line of slots by `position`, gaps allowed; detail cards nest under a `parent` up to `depth` 2 (3 levels total). Moving a card carries its detail subtree along (layout derives from `parent`). `tint` is an optional per-card colour (from a fixed palette) applied as the card's border. Uses a custom template with SortableJS drag-drop (top cards between slots/insert-bars; detail cards reorder and reparent within/across stacks); edits use the standard alive edit-locks and broadcast live to all players.
